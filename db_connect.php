...
$userProjectsQuery = "SELECT * FROM categories WHERE author_id = 1";
...
if ($categories->num_rows > 0) {
    ...
    while ($row = $categories->fetch_assoc()) {
        echo "Назва проєкту: " . $row["name"] . "<br>";
        // Додайте інші необхідні поля проєкту
    }
} else {
    echo "Немає проєктів для поточного користувача";
}
    return $categories;
}
...
$userTasksQuery = "SELECT * FROM tasks WHERE author_id = 1";
...
if ($tasks->num_rows > 0) {
    ...
    while ($row = $tasks->fetch_assoc()) {
        echo "Назва завдання: " . $row["title"] . "<br>";
        // Додайте інші необхідні поля завдання
    }
} else {
    echo "Немає завдань для поточного користувача";
}
    return $tasks;
}
